 ...
 *
 * And for the person who types in the terminal instead: what they have
 * selected is offered to the host as context for their next message (a pill in
 * its prompt), so "make this blue" means the selection. It is withdrawn when
 * they select nothing, and never sent on its own.
 */

import { askLine, AGENT_ASK_STATUSES, AskBoard, asksMessage, DIGEST, digestDue, digestMessage } from "./asks.mjs";
import { cellId, cellLabel } from "./model.mjs";
...
export class Collaboration {
	...
	constructor({ instanceId, session, agent, editorOpen, prefs, log = () => {}, now = Date.now, timers = true }) {
		...
		/** The selection last offered to the host, as a key, so an unchanged one is not re-sent. */
		this.offered = "";
		this.unsubscribeSession = session.subscribe((event) => {
			if (event.type === "presence") {
				void this.offerSelection(event.presence);
				return;
			}
	...
	close() {
		if (this.offered) void this.agent.pushContext(this.instanceId, []).catch(() => {});
		this.closed = true;
	...
	/** Offer the person's selection to the host as context for their next message. */
	async offerSelection(presence) {
		const ids = presence?.selection ?? [];
		const key = ids.length ? `${presence.page_id}:${ids.join(",")}` : "";
		if (key === this.offered) return;
		this.offered = key;
		const page = ids.length && presence.page_id ? this.session.document.page({ page_id: presence.page_id }) : null;
		const attachments = [];
		if (page) {
			const wanted = new Set(ids);
			const labels = page
				.drawable()
				.filter((entry) => wanted.has(cellId(entry)))
				.map((entry) => cellLabel(entry).replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim())
				.filter(Boolean);
			const named = labels.slice(0, 3).map((label) => (label.length > 24 ? `${label.slice(0, 23)}…` : label));
			attachments.push({
				title: `${ids.length} selected on "${page.name}"${named.length ? `: ${named.join(", ")}${labels.length > 3 ? ", …" : ""}` : ""}`,
				payload: {
					canvas: "drawio-canvas",
					instanceId: this.instanceId,
					page_id: page.id,
					page: page.name,
					cell_ids: ids.slice(0, 50),
					note: "What the person had selected on the draw.io canvas when they wrote this. \"This\"/\"these\" means these cells; read them with get_diagram cell_ids.",
				},
			});
		}
		try {
			await this.agent.pushContext(this.instanceId, attachments);
		} catch (cause) {
			this.log(`could not offer the selection to the host: ${cause?.message ?? cause}`);
		}
	}

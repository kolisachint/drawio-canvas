...
export class DiagramSession {
	...
	summaryOf(entry) {
		const lines = [...formatPageChanges(entry.pages), ...entry.cells.map(formatCellChange)];
	...
	changesSince(since, { sources = [SOURCE_HUMAN], limit = REPORT_LINES } = {}) {
		...
		for (const entry of this.journal) {
			...
			const entryLines = [...formatPageChanges(entry.pages), ...entry.cells.map(formatCellChange)];
			if (entryLines.length === 0 && entry.label) entryLines.push(entry.label);
			for (const line of entryLines) {
				total += 1;
				if (lines.length < limit) lines.push(`v${entry.version} ${entry.source}${entry.cells[0]?.page ? ` on "${entry.cells[0].page}"` : ""}: ${line}`);
			}
		}
	...
	replace(xml, { source = SOURCE_AGENT, label = "replaced document", seen } = {}) {
		this.document = DrawioDocument.parse(xml);

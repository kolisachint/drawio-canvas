...
/** `#rgb` / `#rrggbb` as 0-255 components, or null for anything else. */
...
/**
 * A readable label colour for text sitting on `background`.
 *
 * Without this, a label takes its colour from the *page's* theme while its
 * shape takes its fill from the *document* — and in dark mode every shape
 * draw.io filled with one of its pastels ends up with near-white text on a pale
 * blue box, which is the first thing anyone notices and the last thing they
 * blame on the theme. A cell that names its own `fontColor` is untouched; this
 * only decides what "no colour specified" means.
 *
 * Relative luminance, sRGB coefficients, the usual threshold.
 */
export function inkOn(background) {
	...
	return luminance > 0.55 ? "#12161c" : "#f5f7fa";
}
...
function renderVertex(entry, box, options) {
	...
	// The label sits on the shape, so it takes its default colour from the fill
	// rather than from the page. See `inkOn`.
	const painted = shape === "text" || keys.fillColor === "none" ? null : (styleColor(keys, "fillColor") ?? options.fill);
	const labelOptions = painted ? { ...options, fontColor: inkOn(painted) ?? options.fontColor } : options;
	...
	const label = renderLabel(cellLabel(entry), keys, box, shape, labelOptions);

...
export const WRITABLE_EXTENSIONS = new Set([".drawio", ".xml", ".svg", ".png"]);
...
export function resolveInWorkspace(workspace, candidate) {
	...
	if (typeof candidate !== "string" || candidate.trim() === "") {
		throw new DiagramError("invalid_input", 'A workspace-relative file path is required, e.g. "docs/architecture.drawio".');
	}
...
export async function readDiagramFile(absolutePath, { shown = absolutePath } = {}) {
	let content;
	try {
		content = await readFile(absolutePath, "utf8");
	} catch (cause) {
		// Node's errno codes (ENOENT, EISDIR) are not codes a model can act on, and
		// their messages carry the absolute path; say it in workspace terms.
		if (cause?.code === "ENOENT") throw new DiagramError("file_not_found", `No file "${shown}" in the workspace.`);
		if (cause?.code === "EISDIR") throw new DiagramError("not_a_file", `"${shown}" is a directory; give the path of a .drawio file inside it.`);
		if (cause?.code === "EACCES" || cause?.code === "EPERM") throw new DiagramError("file_unreadable", `"${shown}" cannot be read (permission denied).`);
		throw cause;
	}
	const trimmed = content.trim();
	if (trimmed.length === 0) throw new DiagramError("empty_file", `"${shown}" is empty.`);
	...
	let document;
	try {
		document = DrawioDocument.parse(trimmed);
	} catch (cause) {
		if (cause instanceof DiagramError) throw new DiagramError(cause.code, `"${shown}" is not a draw.io diagram: ${cause.message}`);
		throw cause;
	}
...
/** Write text (or a PNG's bytes) to a path inside the workspace, creating the directory if needed. */
 ...
 */
export async function writeSnapshot(instanceId, { xml, filePath, port, token, asks }) {
	const target = snapshotPath(instanceId);
	if (!target) return false;
	try {
		await mkdir(SNAPSHOT_DIR, { recursive: true, mode: 0o700 });
		// The port and token let the reopened instance keep the person's tab: same
		// URL, and the page's event stream reconnects by itself. Owner-only file,
		// removed as soon as it is read back.
		await writeFile(target, JSON.stringify({ xml, filePath: filePath ?? null, port: port ?? null, token: token ?? null, asks: asks ?? null }), { encoding: "utf8", mode: 0o600 });
		return true;
	} catch {
		return false;
	}
...
export async function takeSnapshot(instanceId) {
	...
	try {
		...
		return parsed && typeof parsed === "object" && (typeof parsed.xml === "string" || typeof parsed.token === "string") ? parsed : null;
	} catch {
		return null;
	}
}

 ...
 * presence, the agent's requests to the editor (`rpc`: screenshot, export,
 * focus, layout), which the page answers on `POST /api/rpc`, and `collab`: the
 * agent's status and the person's asks (`lib/collab.mjs`), which the page
 * changes on `POST /api/collab`.
 */
 ...
 */
export async function startInstanceServer({ session, token, extensionDir, drawio, log, port: wantedPort = 0 }) {
	/** Changes every time a server starts, so a reconnecting page can tell it is talking to a new one. */
	...
	/** The instance's collaboration controller, set once it exists (see `setCollab`). */
	let collab = null;
	...
	async function handle(request, response) {
		...
		if (get && route === "/api/collab") return sendJson(response, 200, collab ? collab.state() : { unavailable: true });
		if (postTo("/api/collab")) {
			if (!collab) return sendJson(response, 503, { error: "The canvas is still starting.", code: "not_ready" });
			try {
				return sendJson(response, 200, await collab.command(await body()));
			} catch (cause) {
				throw cause instanceof DiagramError ? cause : new DiagramError(cause?.code ?? "invalid_input", cause?.message ?? String(cause));
			}
		}
	...
	async function handleFile(input) {
		...
		if (input.op === "open") {
			const { xml, inflatedPages } = await readDiagramFile(absolute, { shown: relative });
	...
	function streamEvents(request, response, role) {
		...
		response.write(`event: hello\ndata: ${JSON.stringify({ version: session.version, epoch })}\n\n`);
	...
	// A reload asks for the port the person's tab is on. If something else has it
	// by now, a fresh one: the tab is lost, the canvas is not.
	try {
		await listen(wantedPort);
	} catch (cause) {
		if (!wantedPort) throw cause;
		log?.(`port ${wantedPort} is taken (${cause.code ?? cause.message}); serving on a new one, so the open tab must be reopened`);
		await listen(0);
	}

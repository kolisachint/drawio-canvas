...
export async function startInstanceServer({ session, token, extensionDir, drawio, log }) {
	 ...
	 */
	function requestEditor(method, params = {}, { timeoutMs = RPC_TIMEOUT_MS } = {}) {
		const editor = [...clients].filter((client) => client.role === "editor").at(-1);
		if (!editor) {
			return Promise.reject(
				new DiagramError("no_editor", "No draw.io editor is open for this canvas. Ask the person to open the canvas URL, then try again."),
			);
		}
		const id = randomUUID();
		return new Promise((resolve, reject) => {

...
export function createDrawioCanvas({ createCanvas, CanvasError }, { extensionDir, log = () => {}, drawio, prepare = true }) {
	...
	function inputOf(action, input) {
		...
		if (input !== undefined && input !== null && (typeof input !== "object" || Array.isArray(input))) {
			throw new CanvasError("invalid_input", `${action.name}: input must be an object matching the action's inputSchema, got ${Array.isArray(input) ? "an array" : typeof input}.`);
		}
		// Only what a handler would otherwise trip over as a TypeError: missing
		// required fields, and arrays that are not arrays. Anything subtler is the
		// handler's to judge.
		const schema = action.inputSchema ?? {};
		for (const key of schema.required ?? []) {
			if (input?.[key] === undefined) throw new CanvasError("invalid_input", `${action.name}: input.${key} is required.`);
		}
		for (const [key, property] of Object.entries(schema.properties ?? {})) {
			if (property?.type === "array" && input?.[key] !== undefined && !Array.isArray(input[key])) {
				throw new CanvasError("invalid_input", `${action.name}: input.${key} must be an array, got ${typeof input[key]}.`);
			}
		}
		return input;
	}
	...
	const canvas = {
		...
		description:
			...
			"Speed (measured): reads and edits are local, 1-10 ms, and appear in their editor in about 0.3 s. search_shapes about 0.1 s. focus, layout, screenshot and .png export go through their open draw.io tab, 0.05-0.5 s, and fail with no_editor when no tab is open. " +
